...
// Function: feature for users to open/close all FAQ accordions
function openCloseFAQAccordionsMulti() {
	// Classes on _markup-3x.kit (three groups of accordions), template = nha-subpage-faq-multi
	// Non-admin: each group has it's own open button
	const allFAQAccordions1 = document.querySelectorAll('[js-accordion-multi~="1"]');
	const allFAQAccordions2 = document.querySelectorAll('[js-accordion-multi~="2"]');
	const allFAQAccordions3 = document.querySelectorAll('[js-accordion-multi~="3"]');
	const openCloseFAQButton1 = document.querySelector('[js-accordions-button-open~="1"]');
	const openCloseFAQButton2 = document.querySelector('[js-accordions-button-open~="2"]');
	const openCloseFAQButton3 = document.querySelector('[js-accordions-button-open~="3"]');

	// Admin: one button that opens all
	const allFAQAccordionsToHide = document.querySelectorAll(".js-hide-for-admin");
	const openCloseFAQButtonCMS = document.querySelector('[js-accordions-button-open~="admin"]');
	const allFAQAccordionsMulti = document.querySelectorAll("[js-accordion-multi]");

	// Function: open all accordions
	function openFAQAccordions(accordions, button) {
	...
	function closeFAQAccordions(accordions, button) {
		accordions.forEach(function (accordion) {
	...
	// Function: click listener on open/close buttons
	function buttonClickListener(groupAccordions, groupButton) {
		groupButton.addEventListener("click", function () {
			if (groupButton.textContent.includes("Expand")) {
				openFAQAccordions(groupAccordions, groupButton);
			} else {
				closeFAQAccordions(groupAccordions, groupButton);
			}
		});
	}

	buttonClickListener(allFAQAccordions1, openCloseFAQButton1);
	buttonClickListener(allFAQAccordions2, openCloseFAQButton2);
	buttonClickListener(allFAQAccordions3, openCloseFAQButton3);
	buttonClickListener(allFAQAccordionsMulti, openCloseFAQButtonCMS);

	// Event: listen for Ctl/Cmd + F and automatically open all accordions
	document.addEventListener("keydown", function (event) {
		const platform = navigator.userAgent;
		if ((platform.includes("Mac") && event.metaKey && event.key === "f") || (platform.includes("Win") && event.ctrlKey && event.key === "f")) {
			if (isAdmin()) {
				openFAQAccordions(allFAQAccordionsMulti, openCloseFAQButtonCMS);
			} else {
				openFAQAccordions(allFAQAccordions1, openCloseFAQButton1);
				openFAQAccordions(allFAQAccordions2, openCloseFAQButton2);
				openFAQAccordions(allFAQAccordions3, openCloseFAQButton3);
			}
		}
	});

	if (isAdmin()) {
		allFAQAccordionsToHide.forEach(function (accordion) {
