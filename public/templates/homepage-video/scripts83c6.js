...
function homepageCarousel() {
	...
	function gotoSlide(target, start) {
		...
		currentSlide = target;
		let sliderParent = document.querySelector("[slider='slide'][num='" + target + "']").parentElement;

		while (sliderParent && (!sliderParent.hasAttribute("slider") || sliderParent.getAttribute("slider") != "set")) {
			sliderParent = sliderParent.parentElement;
		}

		if (sliderParent && sliderParent.hasAttribute("slider") && sliderParent.getAttribute("slider") === "set") {
	...
	const enableSwipe = () => {
		...
		let gestureZone = [...document.getElementsByClassName("js-active-slide")];
		...
		function handleGesture(touchstartX, touchstartY, touchendX, touchendY) {
			...
			if (Math.abs(xDirection) > Math.abs(yDirection)) {
				...
				if (xDirection > 0) {
					...
					gotoSlide(target);
					gestureZone = [...document.querySelectorAll(`[num="${target}"]`)];
				} else {
					const target = Number(currentSlide) + 1;
					gotoSlide(target);
					gestureZone = [...document.querySelectorAll(`[num="${target}"]`)];
