...
/////////////////BambooHR for jobs page

//hide 'Powered By...'
function hidePoweredBy() {
    const bambooHRDiv = document.querySelector("#BambooHR-ATS");
    const poweredBy = bambooHRDiv.nextElementSibling;
    if (poweredBy) {
        poweredBy.remove();
    }
}
...
async function modalCatalog() {
    ...
    ); // Parent element for SMS checkbox
...
async function modalDigital() {
    ...
    function marketoSubmitFormDigital() {
        ...
        window.open(
            "https://issuu.com/natural-habitat-adventures/docs/2025-26_travel-catalog_nat-hab?fr=sYmI5YTc2OTAwNDU",
            "_blank"
        );
    }
}
...
function modalYoutube() {
    ...
    const youtubeLinks = document.querySelectorAll(
        "a[href*='youtube.com']:not([nomodal])"
    );
...
try {
    ...
    if (urlPathname.includes("/careers/")) {
