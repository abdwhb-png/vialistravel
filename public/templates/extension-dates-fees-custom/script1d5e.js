...
autoPopupChatAccordionClickListener(); // lives in dev/components/trip/dates-pricing/_script.js

function hidePastCustomPricingAccordions() {
	$("[js-pricing-accordion] label").each(function () {
		let priceYear = this.getAttribute("for");
		priceYear = parseInt(priceYear.replace("-fees", ""));
		const currentYear = new Date().getFullYear();
...
try {
	hidePastCustomPricingAccordions();
} catch (error) {
	console.error(error);
}
