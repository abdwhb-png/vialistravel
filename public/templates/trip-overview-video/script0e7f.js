...
function initPhotoTours() {
	if (isAdmin()) {
		document.querySelector('[photo-tours="wrapper"]').classList.remove("hide");
	}
}
...
function addFromOnTripPricingBlurb() {
	try {
		const pricingBlurbElement = document.querySelector("[pricing~='wrapper'].show");

		if (pricingBlurbElement) {
			const pricingTextElement = pricingBlurbElement.querySelector("[pricing~='cost']");
...
function pricingBlurbPrivateOrCustom() {
	try {
		...
		if (urlPathname.includes("/africa/")) {
			makeItPrivateHref = document.querySelector("[pricing~='private'] a.custom");
		} else {
			makeItPrivateHref = document.querySelector("[pricing~='private'] a.private");
		}
...
function physicalModal() {
	...
	function checkUrl() {
		if (document.URL.includes("#physical") || window.location.search.match(/lightbox\=physical/i)) {
			label.click();
...
function trip360Modals() {
	try {
		...
		const trip360ModalClose = document.querySelector("[trip-360-modal-close]");
		const trip360Links = document.querySelectorAll("a[href*='?lightbox360']");
		...
		document.addEventListener("click", function (event) {
			if (trip360Modal.contains(event.target) || (event.target === trip360Modal && trip360Modal.open)) {
				closeTrip360Modal();
			}
		});
...
// try {
// 	openGenericModalIfLightboxSearchParamInURL("/-lightboxes-carbon/");
// } catch (error) {
// 	console.error(error);
// }
