...
// Function: show either register or video link for webinars
function showWebinarLink() {
	const webinarLinkElements = document.querySelectorAll("[js-webinar-link]");
	const webinarDays = document.querySelectorAll("[js-day]");
	const allWebinarDates = document.querySelectorAll("[js-webinar-date]");
	const currentDate = new Date();
	const currentYear = currentDate.getFullYear();
	const mountainTime = new Date().toLocaleString("en-US", { timeZone: "America/Denver" });
	const currentMountainTime = new Date(mountainTime);

	// make the entire webinar field clickable and go to the active link
	webinarDays.forEach(function (day) {
		day.addEventListener("click", function () {
			let link = day.querySelector("[js-webinar-link]");
			let option = link.getAttribute("js-webinar-link");
			let linkToClick = link.querySelector(`[js-webinar~="${option}"]`);

			if (linkToClick && !linkToClick.classList.contains("js-webinar-registration-inactive") && !day.classList.contains("js-enso-on")) {
				linkToClick.click();
			}
		});
	});

	// make register link inactive after 2pm MT on scheduled day
	allWebinarDates.forEach(function (date) {
		// get webinar date and change into date object
		let dateString = date.textContent;
		let fullDate = `${dateString}, ${currentYear} 14:00:00`;
		let parsedDate = Date.parse(fullDate);
		let webinarDate = new Date(parsedDate);

		if (currentMountainTime > webinarDate) {
			let registerLink = date.parentNode.querySelector('[js-webinar~="register"]');

			registerLink.classList.add("js-webinar-registration-inactive");
			registerLink.textContent = "Video Coming Soon";
			registerLink.addEventListener("click", function (e) {
				e.preventDefault();
			});
			date.parentNode.classList.add("js-revert-cursor");
		}
	});

	// show the desired link and set active link attribute
	webinarLinkElements.forEach(function (link) {
		let linkOption = link.getAttribute("js-webinar-link");
		let registrationLink = link.querySelector('[js-webinar~="register"]');
		let videoLink = link.querySelector('[js-webinar~="video"]');

		if (registrationLink.getAttribute("href") !== "") {
			if (linkOption === "video") {
				registrationLink.classList.add("hide");
				videoLink.classList.remove("hide");
				link.parentNode.classList.remove("js-revert-cursor");
			} else {
				registrationLink.classList.remove("hide");
				videoLink.classList.add("hide");
			}
		} else {
			link.parentNode.classList.add("js-revert-cursor");
		}
	});
}
