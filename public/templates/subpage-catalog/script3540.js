...
function catalogFormPage() {
	let userCountry = localStorage.getItem("country") || "unkmown";
	let catalogPage = new Object();
	catalogPage.formName = "#form-catalog-page";
	catalogPage.newsletter = false;
	catalogPage.smsElementParent = document.querySelector(".js-sms-checkbox-catalog-form-page");
	catalogPage.smsElement = document.querySelector("[js-catalog-form-page-sms]");
	catalogPage.phoneElementInput = document.querySelector("[js-catalog-form-page-phone-toggle]");
	catalogPage.phoneElementLabel = document.querySelector('[for~="form-catalog-page-phone"]');

	catalogPage.locationSelector = "js-catalog-location";
	catalogPage.locationFirstElement = document.querySelector(`[${catalogPage.locationSelector}]`);
	catalogPage.locationAllElements = document.querySelectorAll(`[${catalogPage.locationSelector}]`);
	catalogPage.locationValue = "URL"; //default
	catalogPage.firstInput = document.querySelector("[catalog-form-page-label]");

	let catalogPageHeadline = document.querySelector(".js-catalog-page-headline"),
		catalogPageScreenRequired = document.querySelector(".js-catalog-page-required"),
		catalogPageScreenOptional = document.querySelector(".js-catalog-page-optional"),
		catalogPageAside = document.querySelector(".js-catalog-page-aside");
	...
	function smsCheckbox() {
		catalogPage.smsElement.addEventListener("input", function () {
			if (this.checked) {
				catalogPage.phoneElementInput.setAttribute("required", "");
				catalogPage.phoneElementLabel.setAttribute("catalog-form-page-label", "required");
			} else {
				catalogPage.phoneElementInput.removeAttribute("required");
				catalogPage.phoneElementLabel.setAttribute("catalog-form-page-label", "");
				catalogPage.phoneElementInput.classList.remove("js-invalid");
			}
		});
	}

	// EVENT - Set location and catalogPage checkbox if country = US, India, or Australia
	const countryInput = document.querySelectorAll(`${catalogPage.formName} .js-smarty-country`);
	...
	if (userCountry === "US") {
		...
		initSmarty(catalogPage.formName);
		catalogPage.smsElementParent.classList.remove("hide");
		smsCheckbox();
	...
	// EVENT - Check if user accessed form via Contact Us page
	if (document.referrer.includes("nathab.com/contact/")) {
		catalogPage.locationValue = "Contact Us";
	}

	pushEvent("Catalog", catalogPage.locationValue, "Open");

	setTimeout(() => {
		catalogPage.firstInput.focus();
	}, 1000);
	...
	document.querySelector(".js-catalog-page-button-required").addEventListener("click", function (e) {
		e.preventDefault();
		postFormCatalog();
	});

	// EVENT - Handle submit of optional form
	document.querySelector(".js-catalog-page-button-optional").addEventListener("click", function (e) {
		e.preventDefault();
		postFormOptional();
	});
	...
	function copyCatalogFieldsToMarketoForm() {
		let fieldsToCopyArray = document.querySelectorAll(`${catalogPage.formName} [mrkto-field]`);
	...
	function pushToMarketoCatalogOptional(formName) {
		let checkboxesToCopyArray = document.querySelectorAll(`${catalogPage.formName} [mrkto-checkbox]`);
	...
	function postFormCatalog() {
		...
		// Validate form
		let isValid = isFormValid(catalogPage.formName);
		if (isValid === false) {
			return;
		}

		// Check for spam
		if (isThisSpam(catalogPage.formName) === true) {
			return;
		}

		toggleButtonDisabled(document.querySelector(".js-catalog-page-button-required"));
		...
		MktoForms2.whenReady(function () {
			...
			copyCatalogFieldsToMarketoForm(catalogPage.formName); // Used for both required and optional

			if (isNewsletterChecked(catalogPage.formName) === true) {
				document.querySelector("#isEmailSignup").value = true;
				optins += " eNews,";
				setOptsOnMarketoForm();
			}

			if (isSMSOptinChecked(catalogPage.formName) === true) {
			...
			pushEvent("Catalog", catalogPage.locationValue, pushEventString);
			pushEventFacebook();
			pushEventBing();

			limitCheckboxes(catalogPage.formName);

			setTimeout(() => {
				catalogPageHeadline.classList.add("hide");
				catalogPageAside.classList.add("hide");
				catalogPageScreenRequired.classList.add("hide");
				catalogPageScreenOptional.classList.remove("hide");
				window.scrollTo(0, 0);
			}, 2750);
		});
	}
	...
	function postFormOptional() {
		...
		toggleButtonDisabled(document.querySelector(".js-catalog-page-button-optional"));
		...
		pushUtmToMarketoForm(); // function lives in global
		pushToMarketoCatalogOptional(catalogPage.formName); // Only used for catalog optional
		if (isNewsletterChecked(catalogPage.formName) === true) {
			document.querySelector("#isEmailSignup").value = true;
			setOptsOnMarketoForm();
		}
		...
		pushEvent("Catalog", catalogPage.locationValue, "Submit Page 2");
		pushEventFacebook();
		pushEventBing();

		setTimeout(function () {
			window.location.href = "/contact/catalog-request-confirmation/";
		}, 3000);
	}
