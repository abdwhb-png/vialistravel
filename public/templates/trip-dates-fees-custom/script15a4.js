		...
		success: function (data) {
			...
			$("[js-pricing-accordion] [js-pricing-summary]").each(function () {
				...
				if (priceYear < tripYears[0]) {
					...
					$(this).append("<em class='admin-note'>— *Admin view only*</em>");
				}
			});
...
function replaceMIPImagesInPricingAccordions() {
	let pricingAccordionMIPImages = document.querySelectorAll("[accordion~='innercontent'] img[src*='MIP']");
...
function hideShowPricingAccordionsForAdmin() {
	...
	pricingAccordions.forEach((accordion) => {
		...
		if (pricingYear + 1 === currentYear && currentMonth < 3) accordion.setAttribute("js-show-for-admin", "block");

		// show this year for admin
		if (pricingYear == currentYear) accordion.setAttribute("js-show-for-admin", "block");

		// show next year for admin
		if (pricingYear == currentYear + 1) accordion.setAttribute("js-show-for-admin", "block");

		// show +2 years out for admin
		if (pricingYear == currentYear + 2) accordion.setAttribute("js-show-for-admin", "block");
	});
}
