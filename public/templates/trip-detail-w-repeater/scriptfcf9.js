...
// Function: show 'Request Info' button on MIC Africa or inline form on Make It Private pages
function showMakeItPrivateOrCustomElements() {
	if (!window.location.pathname.includes("/make-it-private/")) return;
	if (window.location.pathname.includes("/africa/") || window.location.pathname.includes("botswana")) {
		showCustomButton();
	} else {
		showInlineForm("[js-show-private-form]");
	}

	function showInlineForm(form) {
		const contentWrapper = document.querySelector("[content~='wrapper']");
		const inlineForm = contentWrapper.querySelector(form);
		inlineForm.classList.remove("hide");
	}

	function showCustomButton() {
		const requestInfoCustomButton = document.querySelector("[js-show-custom-button]");
		requestInfoCustomButton.classList.remove("hide");
	}
}
...
try {
	if (window.location.pathname.includes("/make-it-private/")) {
		showMakeItPrivateOrCustomElements();
	}
} catch (error) {
	console.error(error);
}
