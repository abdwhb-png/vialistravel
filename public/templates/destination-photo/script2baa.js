...
function destinationNav() {
	const destinationNavEnhanced = document.querySelector("[js-nav-item-pattern]");

	if (destinationNavEnhanced) createNavItems();

	let activeNav = null;
	const html = document.querySelector(".js-scroll-trigger");
	const navItems = document.querySelectorAll("[js-destination-nav]");
	const destinationTitles = document.querySelectorAll("[js-destination-title]");
	const destinationSections = document.querySelectorAll("[js-destination-section]");
	...
	function createNavItems() {
		const headlines = document.querySelectorAll("[js-set-nav-link]");
		let newNavItems = "";

		// Start from the second headline
		for (let headline of headlines) {
			// Skip if the headline has a class of "hide"
			if (headline.classList.contains("hide")) {
				continue;
			}

			let navPattern = destinationNavEnhanced.outerHTML;
			let oldText = destinationNavEnhanced.textContent;
			let newText = headline.textContent;

			navPattern = navPattern.replace(oldText, newText);
			newNavItems += navPattern;
		}

		destinationNavEnhanced.insertAdjacentHTML("afterend", newNavItems);
	}

	// Function: use scrollTrigger plugin to detect scroll and do different stuff whether scrolling up or down
	// Plugin: https://greensock.com/docs/v3/Plugins/ScrollTrigger/
	function handleScroll(direction) {
		// / direction key : -1 up, 1 down
		// Scroll down, look for title entering viewport
		if (direction === 1) {
			if (activeNav) {
				destinationTitles.forEach((title) => {
					// ignore divs with no content, example 3rd repeater on this page not populated: https://www.nathab.com/galapagos/
					if (!title.textContent) return;

					// find the title that's 100% in viewport and less than 40% from top
					if (ScrollTrigger.isInViewport(title, 1.0) && ScrollTrigger.positionInViewport(title, "top") <= 0.4) {
						addActiveNav(title, "title");
					}
				});
			} else assignActiveNav();
		}

		// Scroll up, look for active title getting close to bottom of previous section
		if (direction === -1) {
			if (activeNav) {
				let activeNavID = activeNav.getAttribute("js-destination-nav");

				let activeTitle = document.querySelector(`[js-destination-title="${activeNavID}"]`);
				let previousTitle = document.querySelector(`[js-destination-title~="${activeNavID - 1}"]`);

				// ignore divs with no content, example 3rd repeater on this page not populated: https://www.nathab.com/galapagos/
				if (!activeTitle.textContent) return;

				// if title is more than 65% from top or if the previous title is in the viewport, switch active nav to previous section
				if (ScrollTrigger.positionInViewport(activeTitle, "top") > 0.65 || (previousTitle && ScrollTrigger.isInViewport(previousTitle, 1.0))) {
					if (previousTitle) {
						addActiveNav(previousTitle, "title");
					}
				}
			} else {
				assignActiveNav();
	...
	// Function: no active nav so look for section or title in viewport to assign active nav
	function assignActiveNav() {
		...
		destinationTitles.forEach((title) => {
			let titleID = title.getAttribute("js-destination-title");
			let section = document.querySelector(`[js-destination-section~="${titleID}"]`);
			let positionInViewport = ScrollTrigger.positionInViewport(title, "top");
			let absolutePosition = Math.abs(positionInViewport);

			if (title.classList.contains("hide") || section.classList.contains("hide")) {
				return;
			}

			// determine the best section to make active; important if user doesn't start at top of page, example: refresh from middle of page
			if (!sectionFound) {
				// at least 1 title is in viewport, look for the one positioned in the middle-ish; example page: https://www.nathab.com/alaska-nature-tours/
				if (ScrollTrigger.isInViewport(title, 1.0) && positionInViewport >= 0.06 && positionInViewport <= 0.3) {
					addActiveNav(title, "title");
					sectionFound = true;
					// no title is in viewport, so find the section in the viewport
				} else if (!ScrollTrigger.isInViewport(title, 1.0) && ScrollTrigger.isInViewport(section, 0.1)) {
					addActiveNav(section, "section");
					sectionFound = true;
					// find nearest title for default active state
				} else {
					if (absolutePosition < nearestDistance) {
						nearestDistance = absolutePosition;
						nearestTitle = title;
					}
				}
			}
		...
		// default: set to nearest title so active state gets set on page load if no other criteria are met
		if (!sectionFound) {
			addActiveNav(nearestTitle, "title");
			sectionFound = true;
		}
	}

	// Function: remove active nav state
	function removeActiveNav() {
		if (activeNav) {
			// if one or more nav items have the active class already, remove it so only one active nav at a time
			let jsActiveClasslist = document.querySelectorAll(".js-active-destination");
			if (jsActiveClasslist) {
				jsActiveClasslist.forEach((item) => {
					item.classList.remove("js-active-destination");
				});
			}
		}
	}

	// Function: set active nav state
	function addActiveNav(activeElement, type) {
		removeActiveNav();
		let activeID = activeElement.getAttribute(`js-destination-${type}`);
		let navElement = document.querySelector(`[js-destination-nav~='${activeID}']`);

		navElement.classList.add("js-active-destination");
		activeNav = navElement;
	}

	// Function: look for hash in url; example: http://dev10.nathab.com/us-national-parks-tours/#photo-pro-expeditions scrolls to the photo pro section
	function scrollToSectionsFromURL() {
		let hash = location.hash.substring(1);
		if (!hash) return;

		let navLink = document.querySelector(`.js-${hash}`);

		navLink.click();
	}

	// Function: scrollTo section
	function scrollToSection(sectionTitle, offset) {
		let offsetPosition = sectionTitle.getBoundingClientRect().top + window.scrollY + offset;
		window.scrollTo({ top: offsetPosition, behavior: "smooth" });
	}

	// Function: scroll to section and push GA events
	function doStuffWhenNavClicked(item, offset, attribute) {
		let navItemID = item.getAttribute(attribute);
		let navText = item.textContent;
		let sectionTitle = document.querySelector(`[js-destination-title~='${navItemID}']`);
		let yOffset = offset;
		let navNameToPush;

		addActiveNav(sectionTitle, "title");
		scrollToSection(sectionTitle, yOffset);

		// GA push events
		if (navText.includes("Safaris")) {
			navNameToPush = navText.split("Safaris")[0].trim();
		} else if (navText.includes("Photo")) {
			navNameToPush = "Photo";
		} else {
			navNameToPush = navText;
		}
		pushEvent("Clicks", "Region Leftnav", navNameToPush);
	}

	// Actions for titles:
	const visibleDestinationTitles = Array.from(destinationTitles).filter((title) => {
		const computedStyle = window.getComputedStyle(title);
		return computedStyle.display !== "none";
	});

	visibleDestinationTitles.forEach((title, i) => {
		let titleText = title.innerText;

		// if no title on a section other than first, add class to hide border
		if (titleText.trim() === "") {
			if (i === 0 && !destinationNavEnhanced) {
				// backup in case someone enters a title and then deletes later; enso default value doesn't populate
				title.innerText = "Classic Nature Tours";
			} else {
				title.classList.add("js-no-title");
			}
		}

		// number attributes
		title.setAttribute("js-destination-title", i + 1);
	});

	// Actions for subnav - desktop & mobile:
	navItems.forEach((item, i) => {
		let itemText = item.innerText;
		let formattedText = itemText.toLowerCase().trim().replace(/\s+/g, "-");

		// hide if empty title other than first
		if (!itemText) {
			if (i === 0 && !destinationNavEnhanced) {
				// backup in case someone enters a title and then deletes later; enso default value doesn't populate; gets set above but this runs before it's set
				item.innerText = "Classic Nature Tours";
			} else {
		...
		// number attributes
		item.setAttribute("js-destination-nav", i + 1);

		// set class based on title to use if hash included in url
		item.classList.add(`js-${formattedText}`);

		// add click listener to scroll to title and push GA events
		item.addEventListener("click", () => {
			doStuffWhenNavClicked(item, -150, "js-destination-nav");
		});
	});

	// Action for sections:
	// number attributes
	const visibleDestinationSections = Array.from(destinationSections).filter((section) => {
		const computedStyle = window.getComputedStyle(section);
		return computedStyle.display !== "none";
	});

	visibleDestinationSections.forEach((section, i) => {
		section.setAttribute("js-destination-section", i + 1);
	});

	// Event: listen for scrolling; important to make this call after attribute numbers have been assigned
	// Plugin: https://greensock.com/docs/v3/Plugins/ScrollTrigger/static.create()
	ScrollTrigger.create({
		trigger: html,
		onUpdate: (self) => handleScroll(self.direction)
	});

	// Event: function call to scroll to hash section from url
	window.addEventListener("load", scrollToSectionsFromURL);

	// Event: function call to assign active nav initially (on page load)
	if (activeNav === null) {
		assignActiveNav();
	}
}
...
function addFromOnTripRepeaterPricing() {
	const repeaterPricingElements = document.querySelectorAll("[repeater-trip-pricing].show");
	...
	repeaterPricingElements.forEach((repeater) => {
		...
		if (pricingTextElement) {
			...
			if (pricingTextContent.includes("$")) {
				pricingTextElement.textContent = pricingTextContent.replace(pricingTextContent, `From ${pricingTextContent}`);
			}
		}
	});
}
...
// template componennts try/catch
try {
	manageBreadcrumbs();
} catch (error) {
	console.error(error);
