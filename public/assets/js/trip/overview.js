...
async function setUserCountry() {
    try {
        let response = await fetch(
            "https://ipinfo.io/json?token=32d8daa92e84c3"
        );
...
function initPhotoTours() {
    if (isAdmin()) {
        document
            .querySelector('[photo-tours="wrapper"]')
...
function scrollToDifferences() {
    ...
    let anchorLink = document.querySelector(".js-differences");
    if (hash == "#differences")
...
function checkIfEmpty() {
    const checkIfEmpty = $("[reasons~='text']").text().trim();
    if (!checkIfEmpty.length) {
        $("[reasons~='text']").prev("h2").hide();
    }
}

// Function: allows custom content to show next the calendar icon by adding 'From' to Trip Pricing field only if $
function addFromOnTripPricingBlurb() {
    try {
        const pricingBlurbElement = document.querySelector(
            "[pricing~='wrapper'].show"
        );

        if (pricingBlurbElement) {
            const pricingTextElement =
                pricingBlurbElement.querySelector("[pricing~='cost']");
            const pricingTextContent = pricingTextElement.textContent;

            if (pricingTextContent.includes("$")) {
                pricingTextElement.textContent = pricingTextContent.replace(
                    pricingTextContent,
                    `From ${pricingTextContent}`
                );
            }
        }
    } catch (error) {
...
// Function: show Make it Private (non-Africa trips) or Make it Custom (Africa trips)
function pricingBlurbPrivateOrCustom() {
    try {
        let makeItPrivateHref;
        const urlPathname = location.pathname;

        if (urlPathname.includes("/africa/")) {
            makeItPrivateHref = document.querySelector(
                "[pricing~='private'] a.custom"
            );
        } else {
            makeItPrivateHref = document.querySelector(
                "[pricing~='private'] a.private"
            );
        }
        makeItPrivateHref.classList.remove("hide");
    } catch (error) {
        console.log(error);
    }
}

// Trying this approach to avoid global variable conflicts with other modals.
function physicalModal() {
    let links = document.querySelectorAll("a[href*='#physical']");
    let label = document.querySelector("[js='modal-physical-hook']");
    // grab url from window & parse --> guaranteed that it'll end in /dates-fees/
    const urlPathname = window.location.pathname;

    let pathnameArray = urlPathname.split("/");
    pathnameArray = pathnameArray.filter((item) => item !== "");
    let originURL = "../dates-fees/";

    if (pathnameArray.length === 4) {
        originURL = "../../dates-fees/";
    } else if (pathnameArray.length === 2) {
        originURL = "./dates-fees/";
    }

    let targetElement = document.querySelector(".js-load-physical");
    ...
    function checkUrl() {
        if (
            document.URL.includes("#physical") ||
            window.location.search.match(/lightbox\=physical/i)
        ) {
            label.click();
    ...
    async function fetchContent() {
        let result = await fetch(originURL)
            .then(function (response) {
            ...
            .then(function (html) {
                ...
                return doc.querySelector("#physical-requirements").innerHTML;
            })
            .catch(function (err) {
                console.warn("Something went wrong.", err);
            });
...
function buildCarbonModal() {
    const arrayOfHrefForLightboxes = document.querySelectorAll(
        "[href*='/-lightboxes-carbon/']"
    );
    ...
    arrayOfHrefForLightboxes.forEach((selector) => {
        ...
        if (selectorHrefValue.startsWith("https://www.nathab.com")) {
            selectorHrefValue = selectorHrefValue.replace(
    ...
    function carbonModalCloseListener() {
        const carbonModalClose = document.querySelector(
            "[js-click-toggle='carbon']"
        );
        ...
        function openCMS(event) {
            if (event.keyCode == 9) {
                window.location.href =
                    modalURL + "?ensoAction=group&name=group-modal-carbon";
            }
        }
    }
}
...
function trip360Modals() {
    try {
        const trip360Modal = document.querySelector("[trip-360-modal]");
        const trip360ModalClose = document.querySelector(
            "[trip-360-modal-close]"
        );
        const trip360Links = document.querySelectorAll(
            "a[href*='?lightbox360']"
        );
        const trip360Iframe = document.querySelector("[trip-360-modal-iframe]");
        const unsetSrc = "";

        // Event : check for trip-360 links
        function checkLinksForTrip360() {
            trip360Links.forEach((item) => {
                item.addEventListener("click", (event) => {
                    event.preventDefault();
                    if (event.shiftKey) {
                        return;
                    }
                    let trip360Src = item.getAttribute("href");
                    trip360Src = trip360Src.replace(/(^.*\?lightbox360=)/i, "");
                    openTrip360Modal(`${trip360Src}`);
                });
            });
        }
        checkLinksForTrip360();

        // Function - check url for trip-360
        function checkUrlForTrip360() {
            // Example - https://momento360.com/e/uc/63030de2dc894f11a5f7e78bca34ee76?utm_campaign=embed&utm_source=other&size=medium&display-plan=true
            let querystring = window.location.search;
            if (querystring.match(/\?lightbox360/)) {
                let trip360Src = querystring.replace(/(^.*lightbox360=)/i, "");
                trip360Src = decodeURIComponent(trip360Src);
                openTrip360Modal(trip360Src);
            }
        }
        checkUrlForTrip360();

        // Function - Open trip-360 Modal
        function openTrip360Modal(src) {
            setSrc(src);
            if (trip360Modal.hasAttribute("open")) return;
            if (!isDialogSupported()) {
                trip360Modal.setAttribute("open", "");
            } else {
                trip360Modal.showModal();
            }
            sessionStorage.setItem("modalOpen", true);
            toggleAmbientVideo(false);
        }

        // Function - Close trip-360 Modal
        function closeTrip360Modal() {
            setSrc(unsetSrc);
            if (!isDialogSupported()) {
                trip360Modal.removeAttribute("open");
            } else {
                trip360Modal.close();
            }
        }

        // Function - Set trip-360 Src
        function setSrc(src) {
            trip360Iframe.setAttribute("src", src);
        }

        // Event : close event for when modal is open
        document.addEventListener("click", function (event) {
            if (
                trip360Modal.contains(event.target) ||
                (event.target === trip360Modal && trip360Modal.open)
            ) {
                closeTrip360Modal();
            }
        });
        ...
        trip360ModalClose.addEventListener("click", () => {
            closeTrip360Modal();
        });

        // Event : if escape key is pressed (but captures all close events)
        trip360Modal.addEventListener("close", () => {
            closeTrip360Modal();
            sessionStorage.setItem("modalOpen", false);
            toggleAmbientVideo(true);
        });
...
try {
    pricingBlurbPrivateOrCustom();
} catch (error) {
    console.error(error);
}

try {
    addFromOnTripPricingBlurb();
} catch (error) {
    console.error(error);
}

// try {
//     isWistiaVideoPlaying();
// } catch (error) {
//     console.error(error);
// }
