...
function repeaterPrivateOrCustom() {
    let repeaters = document.querySelectorAll(
        "[js-show-private-or-custom].show"
    );
    let repeaterParent;
    let makeItPrivateSpan;

    repeaters.forEach((repeater) => {
        repeaterParent = repeater.closest("a[repeater-trip]");

        if (
            repeaterParent.href.includes("africa") ||
            repeaterParent.href.includes("botswana")
        ) {
...
function addFromOnTripRepeaterPricing() {
    const repeaterPricingElements = document.querySelectorAll(
        "[repeater-trip-pricing].show"
    );
    let pricingTextElement;
    let pricingTextContent;

    repeaterPricingElements.forEach((repeater) => {
        pricingTextElement = repeater.querySelector("[pricing-text]");

        if (pricingTextElement) {
            pricingTextContent = pricingTextElement.textContent;
