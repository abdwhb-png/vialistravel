function isWistiaVideoPlaying() {
    ...
    //     if (
    //         wistiaSrc === "https://fast.wistia.net/embed/iframe/?videoFoam=true"
    //     ) {
    //         return;
    //     }
...
function cleanupAnchorTagsBlankTarget() {
    document.querySelectorAll("a").forEach(function (link) {
        ...
        if (
            linkHref &&
            linkHref.includes("https://app.monstercampaigns.com/")
        ) {
...
function pushEvent(item, action, label) {
    try {
        ...
        console.log(
            "Success: Pushed Event via data layer:",
            item,
            action,
            label
        );
...
function browserSize() {
    ...
    if (
        browserSizeCookie === "blocked" ||
        browserSizeCookie == null ||
...
function setFavIcon() {
    ...
    function updateFavicon() {
        favicon.setAttribute(
            "href",
            mediaQuery.matches
                ? "/assets/images/icons/favicon-dark-mode.png"
                : "/assets/images/icons/favicon-light-mode.png"
        );
    }
    updateFavicon();
...
function manageBreadcrumbs() {
    ...
    if (
        document.querySelector("[js-breadcrumbs-mobile-list]") &&
        document.querySelector("[js-breadcrumbs-mobile-list]").innerText === ""
    ) {
        document
            .querySelector("[js-breadcrumbs-mobile-home-link]")
            .classList.add("hide");
    }
    ...
    if (getBrowserDevice() === "desktop" && breadcrumbsBar) {
        ScrollTrigger.create({
            ...
            onUpdate: (self) =>
                scale_breadcrumbs_pagename(self.direction, breadcrumbsBar),
        });
    }
}
...
function scale_breadcrumbs_pagename(direction, breadcrumbsBar) {
    ...
    // scrolling down
    if (direction === 1 && position < distanceToTop)
        scaleDown(pageName, breadcrumbsBar);

    // scrolling up
    if (direction === -1 && position > distanceToTop)
        scaleUp(pageName, breadcrumbsBar);
}
...
function hideShowPricingAccordionsForAdmin() {
    const pricingAccordions = document.querySelectorAll(
        "[js-pricing-accordion]"
    );
    ...
    pricingAccordions.forEach((accordion) => {
        ...
        // separate out year & make an int
        pricingYear = parseInt(pricingYear.replace("pricing-", ""));

        // show last year for admin until march
        if (pricingYear + 1 === currentYear && currentMonth < 3)
            accordion.setAttribute("js-show-for-admin", "block");

        // show this year for admin
        if (pricingYear == currentYear)
            accordion.setAttribute("js-show-for-admin", "block");

        // show next year for admin
        if (pricingYear == currentYear + 1)
            accordion.setAttribute("js-show-for-admin", "block");

        // show +2 years out for admin
        if (pricingYear == currentYear + 2)
            accordion.setAttribute("js-show-for-admin", "block");
    });
}
...
function olarkLoader() {
    ...
    document.addEventListener("readystatechange", (event) => {
        if (event.target.readyState === "complete") {
            const chatButtonDesktop = document.querySelector(
                ".js-chat-button-desktop"
            );
            const chatButtonMobileBanner = document.querySelector(
                ".js-chat-banner-mobile"
            );
            const chatButtonMobile = document.querySelector(
                ".js-chat-button-mobile"
            );
            const chatMobileDrawerLabel = document.querySelector(
                "[js-click-toggle~='contact']"
            );
            const chatMobileDrawerToggle =
                document.querySelector("[js-close-chat]");
            const chatButtonDesktopClose = document.querySelector(
                ".js-chat-button-desktop--close"
            );
            ...
            olark("api.box.onExpand", function () {
                ...
                if (showConsent === "false") {
                    const consentMessageDiv = document.querySelector(
                        ".olark-gdpr-consent-message"
                    );
            ...
            function preventChatFromMisbehaving() {
                ...
                let listenForClassChanges = new MutationObserver(function (
                    mutations
                ) {
                ...
                let listenForElementRemoval = new MutationObserver(function (
                    mutations
                ) {
                    mutations.forEach(function (mutation) {
                ...
                listenForClassChanges.observe(chatButtonDesktopClose, {
                    attributes: true,
                });
                listenForClassChanges.observe(chatButtonDesktop, {
                    attributes: true,
                });

                // Initiate observer on body (parent element of chat wrapper)
                listenForElementRemoval.observe(bodyDiv, {
                    subtree: true,
                    childList: true,
                });
            }
    ...
    function shouldEmailConsentBeDisplayed() {
        ...
        const consentButton = document.querySelector(
            ".olark-gdpr-consent-checkbox"
        );

        if (
            consentButton &&
            consentButton.classList.contains("olark-gdpr-consent-checked")
        ) {
            consentMessageDiv.remove();
        }
        const onlineStartChatButton = document.querySelector(
            ".olark-button.olark-survey-form-submit"
        ); // when chat is online, this exists and email consent is displayed
        const offlineNextButton = document.querySelector(
            ".olark-button.olark-survey-form-nav"
        ); // when chat is offline, this exists and email consent is not displayed
    ...
    function preCheckConsent(chatStatus) {
        ...
        const privacyPolicy =
            '<a class="js-chat-button-privacy" href="https://www.nathab.com/privacy-policy/" target="_blank">Privacy Policy</a>';
        ...
        function insertPrivacyPolicy() {
            const consentMessageDiv = document.querySelector(
                consentMessageDivClass
            );
            const privacyPolicyElement = document.querySelector(
                ".js-chat-button-privacy"
            );
            if (!privacyPolicyElement) {
                consentMessageDiv.insertAdjacentHTML(
                    "beforeend",
                    privacyPolicy
                );
            }
    ...
    function offlinePageTwo() {
        document
            .querySelector(".olark-button")
            .removeEventListener("click", offlinePageTwo);
        ...
        function iconSwitch() {
            ...
            const enewsIcon =
                '<img class="js-chat-enews-icon" ignore-cdn src="https://cdn.filestackcontent.com/A6dTpd53SmIg0pBfJJhgAz/cache=expiry:31536002/https://www.nathab.com/uploaded-files/_global/thumbnail-enews-v11.png"></img>';
            const enewsIconElement = document.querySelector(
                ".js-chat-enews-icon"
            );
            if (!enewsIconElement) {
    ...
    function autoPopChatAfterXSeconds(isOperatorAvailable) {
        ...
        if (
            getCookie("chatAutoPopupBlock") === "true" ||
            chatCookieCheck < autopopDelay
        ) {
            return;
        }
            ...
            setTimeout(function () {
                ...
                if (modalAndVideoCheck === true) {
                    bodyDiv.addEventListener("click", () => {
                        modalAndVideoCloseTimerForAccordion(
                            isOperatorAvailable
                        );
                    });
                    return;
...
function removeAccordionClickListener() {
    const datesAndPricingAccordions = document.querySelectorAll(
        '[accordion="checkbox"]'
    );
...
function cookieValueCheck(cookie) {
    ...
    if (cookie === "chatAutoPopupSession") {
        currentCookieValue =
            sessionStorage.getItem("chatAutoPopupSession") || currentTime;
    } else {
...
function openChat(isOperatorAvailable) {
    ...
    waitUntilElementExists(chatButtonStartChatClass).then(
        chatStartClickListener
    );

    function chatStartClickListener() {
        const chatButtonStartChat = document.querySelector(
            chatButtonStartChatClass
        );
...
function autoPopupChatAccordionClickListener() {
    const datesAndPricingAccordions = document.querySelectorAll(
        '[accordion="checkbox"]'
    );
    datesAndPricingAccordions.forEach(function (accordion) {
...
function enso() {
    ...
    for (const element of ensoElements) {
        let ensoOverlayContentElement = element.closest(
            ".enso-overlay-content"
        );
...
function isDialogSupported() {
    ...
    // Return boolean
    return (
        typeof dialogPrototype.show === "function" &&
        typeof dialogPrototype.close === "function"
    );
}
...
async function check_url_for_generic_modal() {
    ...
    if (genericQueryPatterns.some((pattern) => pattern.test(querySearch))) {
        ...
        if (!targetModal) {
            targetModal = await fetch_generic_modal_dialog(
                querySearch,
                targetModalSelector
            );
        }
...
async function fetch_generic_modal_dialog(querySearch, targetModalSelector) {
    try {
        ...
        if (!modal) {
            throw new Error(
                `Modal with selector "${targetModalSelector}" not found in the inserted HTML.`
            );
        }
        cdnifyImages();
        return modal;
...
function openModalsFromShortQuerystring() {
    ...
    if (modalTypes.includes(modalParam)) {
        let modal = document.querySelector(
            `[href*='/-lightboxes/'][href*='${modalParam}']`
        ); // this is looking for a link to lightbox and the param word in it
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            var sortData = function (trip) {
                if (
                    window.location.href.indexOf(`${trip.repeaterURL}`) > -1 &&
                    trip.repeaterURL !== ""
                ) {
                    pageData.push(trip);
                }
            ...
            pageData.forEach(function (trip) {
                ...
                trip.departure = trip.departure.replace(
                    oldMonth,
                    months[formatMonthDepart]
                );
                trip.return = trip.return.replace(
                    oldMonthReturn,
                    months[formatMonthReturn]
                );
                if (
                    trip.notes.includes("lightbox") &&
                    trip.notes.includes("www.nathab.com")
                ) {
                    trip.notes = trip.notes.replace(
            ...
            var sortSameDates = function () {
                for (let i = 0; i < pageData.length; i++) {
                    ...
                    if (i + 1 == pageData.length) {
                        ...
                        if (
                            pageData[i].departure === pageData[i + 1].departure
                        ) {
            ...
            var buildAccordions = function (tripyear, index) {
                if (index === 0) {
                    accYear.innerHTML = accYear.innerHTML.replace(
                        "${tripYears[x]}",
                        `${tripyear}`
                    );
                    ...
                    $(accYear).attr(
                        ...
                        $(accYear)
                            .attr("for")
                            .replace("dates-next-year", `dates-${tripyear}`)
                    );
                    $(accInput).attr(
                        "id",
                        $(accInput)
                    ...
                    $(tripContainer).attr(
                        ...
                        $(tripContainer)
                            .attr("class")
                            .replace("trip-dates", `trip-dates-${tripyear}`)
                    );
                } else {
                    accYear.innerHTML = accYear.innerHTML.replace(
                        `${tripYears[index - 1]}`,
                ...
                document
                    .querySelector('[wrapper="dates-accordion"]')
                    .append(newAcc);
                $(".js-append-accordion").first().remove();
                var tripBody = document.querySelector(".js-append-tr");
                var tripHint = document.querySelector(
                    `.trip-dates-hint-${tripyear}`
                );
                var tripFinePrint = document.querySelector(
                    `.trip-dates-fine-print-${tripyear}`
                );
                ...
                if (titleYear == tripyear) {
                    tripHint
                        ? document
                              .querySelector(`.trip-dates-${tripyear}`)
                              .prepend(tripHint)
                        : null;
                    document
                        .querySelector(`.trips-${tripyear}`)
                        .append(dateTripFrag);
                    tripFinePrint
                        ? document
                              .querySelector(`.trip-dates-${tripyear}`)
                              .append(tripFinePrint)
                        : null;
                }
            };
            ...
            $(".js-trip-data").hover(
                function () {
                    ...
                    $(".price-active [trip-body~='price']").addClass(
                        "show-price"
                    );
                },
                function () {
                    $(".price-active [trip-body~='price']").removeClass(
                        "show-price"
                    );
            ...
            $(".price").each(function (i, price) {
                if (price.innerHTML.includes("Cat")) {
                    price.innerHTML = price.innerHTML.replace(/;\s?/g, "<br>");
                }
            });
            ...
            var placeholderToggle = document.querySelector(
                ".js-placeholder-toggle"
            );
            ...
            $("[js-pricing-accordion] [js-pricing-summary]").each(function () {
                ...
                if (priceYear < tripYears[0]) {
                    $(this)
                        .parent("[js-pricing-accordion]")
                        .attr("hide", "all");
                    $(this).append(
                        "<em class='admin-note'>— *Admin view only*</em>"
                    );
                }
            });
...
function replaceMIPImagesInPricingAccordions() {
    let pricingAccordionMIPImages = document.querySelectorAll(
        "[accordion~='innercontent'] img[src*='MIP']"
    );
    if (pricingAccordionMIPImages) {
        pricingAccordionMIPImages.forEach((image) => {
            let privateButton = document.createElement("label");
            privateButton.textContent = "Make It Private";
            privateButton.setAttribute("private", "accordion-button button");
            privateButton.setAttribute("for", "modal-private");
            privateButton.setAttribute("js-private-location", "PRIVATE");

            image.parentNode.insertBefore(privateButton, image);
            image.remove();
        });
    }
}
...
function removeActiveFromOtherLi() {
    let activeLiElements = document.querySelectorAll(
...
function showBookOrCatalogOnMobileNav() {
    const isBookOff = document.querySelector(".booknowoff");
    const isAllBookOff = document.querySelector(".disallow-book-now");
    const navbarMobileCatalog = document.querySelector(
        "[nav-site-link--mobile~='catalog']"
    );
    const navbarBook = document.querySelector(
...
function setSessionsForBackToTripBar() {
...
function initSmarty(form) {
    ...
    function smartySuggestions(search, smarty) {
        ...
        fetch(
            `https://us-autocomplete-pro.api.smartystreets.com/lookup?key=91583922427951116&search=${search}&max_results=5`,
            {
                method: "GET",
                headers: {
        ...
        street.addEventListener("keyup", function (event) {
            ...
            if (event.keyCode === 38) {
                ...
                if (street.nextElementSibling.hasChildNodes() === true) {
                    ...
                    street.nextElementSibling.children[
                        current
                    ].classList.remove("focus");
                    current--;
                    street.nextElementSibling.children[current].classList.add(
                        "focus"
                    );
                }
                ...
                if (street.nextElementSibling.hasChildNodes() === true) {
                    if (
                        current ===
                        street.nextElementSibling.children.length - 1
                    ) {
                        return;
                    }
                    if (current > -1) {
...
function isNewsletterChecked(formName) {
    return document.querySelector(`${formName} ${formName}-requestNewsletter`)
        .checked;
}
...
function pushUtmToMarketoForm() {
    try {
        document.querySelector("#utmcampaign").value =
            utmObject.utmcampaign || "";
        document.querySelector("#utmcontent").value =
            utmObject.utmcontent || "";
...
function limitCheckboxes(form) {
    ...
    arrayOfCheckboxLists.forEach((checkboxList) => {
        ...
        let arrayOfCheckboxes = checkboxList.querySelectorAll(
            "input[type='checkbox']"
        );
        let totalChecked = 0;
...
function clearOMSessionStorage() {
    sessionStorage.setItem("preventOMCta", "false");
}

function pageVisitCounter(cookieName, urlPath) {
...
async function itineraryModal() {
    ...
    itinerary.smsElementParent = document.querySelector(
        ".js-sms-checkbox-itinerary"
    );
    itinerary.smsElement = document.querySelector("[js-itinerary-sms]");
    itinerary.phoneElementInput = document.querySelector(
        "[js-itinerary-phone-toggle]"
    );
    itinerary.phoneElementLabel = document.querySelector(
        '[for~="form-itinerary-phone"]'
    );
    itinerary.catalogElement = document.querySelector(
        "[js-itinerary-catalog-toggle]"
    );
    itinerary.locationSelector = "js-itinerary-location";
    itinerary.locationFirstElement = document.querySelector(
        `[${itinerary.locationSelector}]`
    );
    itinerary.locationAllElements = document.querySelectorAll(
        `[${itinerary.locationSelector}]`
    );

    itinerary.catalogCheckbox = document.querySelectorAll(
        "[js-itinerary-switch]"
    );
    ...
    let catalogFieldWrappersForItinerary = document.querySelectorAll(
            "[js-itinerary-toggle-catalog]"
        ),
        catalogRequiredFieldsForItinerary = document.querySelectorAll(
            "[js-itinerary-field-required]"
        ),
        itineraryScreenRequired = document.querySelector(
            ".js-itinerary-required"
        ),
        itineraryScreenThankYou = document.querySelector(
            ".js-itinerary-thankyou"
        );
    ...
    function smsCheckbox() {
        itinerary.smsElement.addEventListener("input", function () {
            if (this.checked) {
                ...
                itinerary.phoneElementLabel.setAttribute(
    ...
    itinerary.locationAllElements.forEach((element) => {
        element.addEventListener("click", function () {
            ...
            pushEvent("itinerary", "Itinerary", "Open");
    ...
    document
        .querySelector(".js-itinerary-button-required")
    ...
    function copyItineraryFieldsToMarketoForm() {
        let fieldsToCopyArray = document.querySelectorAll(
            `${itinerary.formName} [mrkto-field]`
        );
    ...
    function postFormItinerary() {
        ...
        toggleButtonDisabled(
            document.querySelector(".js-itinerary-button-required")
        );
        ...
        MktoForms2.whenReady(function () {
            ...
            document.querySelector("#recentConversionAction").value =
                "Itinerary Pop-Up";
            document.querySelector("#referringWebsite").value =
                utmObject.referringWebsite || "";
            ...
            pushEvent("itinerary", "Itinerary", pushEventString);
    ...
    function marketoSubmitFormItin() {
        if (checkMarketoForm() === false) {
            pushEvent(
                "Marketo",
                "Form",
                "Not Loaded during marketoSubmitForm()"
            );
        }
...
async function customModal() {
    ...
    custom.smsElementParent = document.querySelectorAll(
        ".js-sms-checkbox-custom"
    );
    custom.smsElement = document.querySelector("[js-custom-sms]");
    custom.phoneElementInput = document.querySelector(
        "[js-custom-phone-toggle]"
    );
    custom.phoneElementLabel = document.querySelector(
        '[for~="form-custom-phone"]'
    );
    custom.locationSelector = "js-custom-location";
    custom.locationFirstElement = document.querySelector(
        `[${custom.locationSelector}]`
    );
    custom.locationAllElements = document.querySelectorAll(
        `[${custom.locationSelector}]`
    );
    ...
    custom.smsElement.addEventListener("input", function () {
        if (this.checked) {
            ...
            custom.phoneElementLabel.setAttribute(
                "custom-form-label",
                "required"
            );
        } else {
    ...
    let linksWithLightbox = document.querySelectorAll(
        "a[href~='?lightbox=custom']"
    );
    ...
    if (
        pageURL.indexOf("#custom") > 0 ||
        pageURL.indexOf("#Custom") > 0 ||
        window.location.search.match(/lightbox\=custom/i)
    ) {
        custom.locationFirstElement.click();
    }
    ...
    // EVENT - Handle submit of required form
    document
        .querySelector(".js-custom-button-required")
        .addEventListener("click", function (e) {
            e.preventDefault();
            postFormCustom();
        });
    ...
    function postFormCustom() {
        ...
        toggleButtonDisabled(
            document.querySelector(".js-custom-button-required")
        );
        ...
        MktoForms2.whenReady(function () {
            ...
            document.querySelector("#requestedAt").value = document.URL;
            document.querySelector("#LeadSource").value = "www.nathab.com";
            document.querySelector("#sourceDetail").value =
                "Custom Africa Question";
            document.querySelector("#recentConversionAction").value =
                "Custom Africa Question";
            document.querySelector("#referringWebsite").value =
                utmObject.referringWebsite || "";
...
async function privateModal() {
    ...
    privateModal.smsElementParent = document.querySelectorAll(
        ".js-sms-checkbox-private"
    );
    privateModal.smsElement = document.querySelector("[js-private-sms]");
    privateModal.phoneElementInput = document.querySelector(
        "[js-private-phone-toggle]"
    );
    privateModal.phoneElementLabel = document.querySelector(
        '[for~="form-private-phone"]'
    );
    privateModal.locationSelector = "js-private-location";
    privateModal.locationFirstElement = document.querySelector(
        `[${privateModal.locationSelector}]`
    );
    privateModal.locationAllElements = document.querySelectorAll(
        `[${privateModal.locationSelector}]`
    );
    ...
    function insertTripNameIntoAsideText() {
        const asideTextSelector = document.querySelector(
            ".js-private-aside-text"
        );
        const questionTextSelector = document.querySelector(
            ".js-private-question-text"
        );
    ...
    function smsCheckbox() {
        privateModal.smsElement.addEventListener("input", function () {
            if (this.checked) {
                ...
                privateModal.phoneElementLabel.setAttribute(
                    "private-form-label",
                    "required"
                );
            } else {
    ...
    // Check for #hash or ?lightbox=value in URL
    if (
        pageURL.indexOf("#private") > 0 ||
        pageURL.indexOf("#Private") > 0 ||
        window.location.search.match(/lightbox\=private/i)
    ) {
        privateModal.locationFirstElement.click();
    }
    ...
    // EVENT - Handle submit of required form
    document
        .querySelector(".js-private-button-required")
        .addEventListener("click", function (e) {
            e.preventDefault();
            postFormPrivate();
        });
    ...
    function copyPrivateFieldsToMarketoForm() {
        let fieldsToCopyArray = document.querySelectorAll(
            `${privateModal.formName} [mrkto-field]`
        );
        fieldsToCopyArray.forEach((field) => {
            let mrktoFieldSelector = field.getAttribute("mrkto-field");
            if (field.value) {
        ...
        document.querySelector("#askaQuestionText").value =
            document.querySelector("#form-private-askaQuestionText").value;
    }
    ...
    function postFormPrivate() {
        ...
        toggleButtonDisabled(
            document.querySelector(".js-private-button-required")
        );
...
async function pdfModal() {
    ...
    pdf.locationFirstElement = document.querySelector(
        `[${pdf.locationSelector}]`
    );
    pdf.locationAllElements = document.querySelectorAll(
        `[${pdf.locationSelector}]`
    );
    ...
    let catalogFieldWrappersForPdf = document.querySelectorAll(
            "[js-pdf-toggle-catalog]"
        ),
        catalogRequiredFieldsForPdf = document.querySelectorAll(
            "[js-pdf-field-required]"
        ),
    ...
    function smsCheckbox() {
        pdf.smsElement.addEventListener("input", function () {
            if (this.checked) {
                ...
                pdf.phoneElementLabel.setAttribute(
                    "pdf-form-label",
                    "required"
                );
            } else {
    ...
    pdf.locationAllElements.forEach((element) => {
        element.addEventListener("click", function () {
            ...
            const countryInput = document.querySelectorAll(
                `${pdf.formName} .js-smarty-country`
            );
    ...
    if (
        pageURL.indexOf("#pdf") > 0 ||
        pageURL.indexOf("#pdf") > 0 ||
        window.location.search.match(/lightbox\=pdf/i)
    ) {
    ...
    // EVENT - Handle submit of required form
    document
        .querySelector(".js-pdf-button-required")
        .addEventListener("click", function (e) {
            e.preventDefault();
            postFormpdf();
    ...
    function postFormpdf() {
        ...
        MktoForms2.whenReady(function () {
            ...
            document.querySelector("#recentTripPDFRequest").value =
                pdf.recentTripPDFRequest;
...
async function askModal() {
    ...
    ask.locationFirstElement = document.querySelector(
        `[${ask.locationSelector}]`
    );
    ask.locationAllElements = document.querySelectorAll(
        `[${ask.locationSelector}]`
    );
    ask.locationValue = "Unknown";
    ask.firstInput = document.querySelector("[ask-form-label]");

    let catalogFieldWrappersForAsk = document.querySelectorAll(
            "[js-ask-toggle-catalog]"
        ),
        catalogRequiredFieldsForAsk = document.querySelectorAll(
            "[js-ask-field-required]"
        ),
    ...
    // Check for #hash or ?lightbox=value in URL
    if (
        document.URL.indexOf("#ask") > 0 ||
        document.URL.indexOf("#Ask") > 0 ||
        window.location.search.match(/lightbox\=ask/i)
    ) {
    ...
    // EVENT - Handle submit of required form
    document
        .querySelector(".js-ask-button-required")
        .addEventListener("click", function (e) {
            e.preventDefault();
            postFormAsk();
    ...
    function copyaskFieldsToMarketoForm() {
        ...
        // set hidden form textarea id=" askaQuestionText" to visible form text; fixes ${tripName} not getting populated in Marketo
        document.querySelector("#askaQuestionText").value =
            document.querySelector("#form-ask-askaQuestionText").value;
    }
...
function convertOldSlideshowToNew() {
    ...
    for (const imageContainer of imageContainers) {
        ...
        for (const image of images) {
            ...
            const cdn =
                "https://cdn.filestackcontent.com/A6dTpd53SmIg0pBfJJhgAz/resize=width:1600,fit:max/quality=value:60/cache=expiry:31536000/compress/";
...
function slideshow(arg) {
    ...
    for (const slideshow of slideshows) {
        ...
        function resetSlideshow() {
            const firstThumb = slideshow.querySelector(
                '[slideshow="thumbnail"]'
            );
        ...
        for (const image of imageList) {
            ...
            const cdnString =
                "https://cdn.filestackcontent.com/A6dTpd53SmIg0pBfJJhgAz/resize=width:1600,fit:max/quality=value:75/cache=expiry:31536000/compress/";
        ...
        function buildThumbnails() {
            imageList.forEach((image, i) => {
                ...
                thumbnail.addEventListener("click", function () {
                    ...
                    targetImg = slideshow.querySelector(
                        `[js-image~="${targetId}"]`
                    );
        ...
        function scrollThumbnailIntoView(entries) {
            entries.map((entry) => {
                ...
                let scrollInt = 1;
                let targetThumbnail = slideshow.querySelector(
                    `[target=image-${index}]`
                );
                if (entry.isIntersecting) {
                    ...
                    let targetScrollThumbnail = slideshow.querySelector(
                        `[target="image-${scrollDirection}"]`
                    );
                    ...
                    if (targetScrollThumbnail) {
                        ...
                        if (index > 0) {
                            targetScrollThumbnail.scrollIntoView({
                                behavior: "smooth",
                                block: "nearest",
                            });
                        }
        ...
        function navLeft() {
            ...
            index = index.replace("image-", "");
            targetImg = slideshow.querySelector(
                `[js-image="image-${parseInt(index) - 1}"]`
            );
            if (!targetImg) {
                leftArrow.classList.add("disabled");
            } else {
...
function openModal(modal) {
    ...
    const firstImageOfSlideshow = modal.querySelector(
        "[slideshow='thumbnail']"
    );
...
function setTripNavPosition() {
    ...
    const magicHidden =
        !magicCTA || getCookie("magic") === "hide" ? true : false;
    const buffer = 24;
    ...
    function checkPositions() {
        ...
        let heightTabletBreadcrumbsBar = getElementProperty(
            "[js-tripnav-section-wrapper]",
            null,
            "height"
        );
        ...
        if (
            ...
            window.innerHeight <= heightsNavHero
        ) {
        ...
        if (window.innerHeight > heightsNavMagic) {
            ...
            ScrollTrigger.create({
                trigger: main,
                onUpdate: (self) =>
                    positionMagicCTA(self.direction, main, magicCTA),
            });
        }
    }
        ...
        if (event.target.readyState === "complete") {
            function scrollWatchForMasthead() {
                ...
                function toggleMasthead(direction) {
                    ...
                    let position = ScrollTrigger.positionInViewport(
                        html,
                        "top"
                    );
                    ...
                    if (direction === -1 && position > scrollUpPositionTarget) {
                        ...
                        scrollDownPositionTarget =
                            position + scrollDownPostionBuffer;
                        magicCTA.classList.remove(
                            "js-reposition-magic",
                            "js-default-magic-position"
                        );
                    }
                    ...
                    if (
                        direction === 1 &&
                        position < scrollDownPositionTarget
                    ) {
                        megaReset.checked = true;
                        masthead.classList.add("js-masthead-hide");
                        masthead.classList.remove("js-masthead-show");
                        scrollUpPositionTarget =
                            position + scrollUpPostionBuffer;
                        if (
                            magicCTA.getAttribute("js-position") ===
                            "js-default-magic-position"
                        ) {
...
function mobileSiteNavHandler() {
    ...
    mobileNavToggleLinks.forEach((link) => {
        link.addEventListener("click", function () {
            ...
            let targetElementNameToTitle =
                targetElementName.charAt(0).toUpperCase() +
                targetElementName.slice(1);
            if (targetElementName) {
                pushEvent(
                    "Misc",
                    `${targetElementNameToTitle} Mobile Nav`,
                    "Clicked"
                );
            }
...
async function modalCatalog() {
    ...
    catalog.smsElementParent = document.querySelector(
        ".js-sms-checkbox-catalog"
    );
    catalog.smsElement = document.querySelector("[js-catalog-sms]");
    catalog.phoneElementInput = document.querySelector(
        "[js-catalog-phone-toggle]"
    );
    catalog.phoneElementLabel = document.querySelector(
        '[for~="form-catalog-phone"]'
    );
    catalog.locationSelector = "js-catalog-location";
    catalog.locationFirstElement = document.querySelector(
        `[${catalog.locationSelector}]`
    );
    catalog.locationAllElements = document.querySelectorAll(
        `[${catalog.locationSelector}]`
    );
    ...
    function smsCheckbox() {
        catalog.smsElement.addEventListener("input", function () {
            if (this.checked) {
                ...
                catalog.phoneElementLabel.setAttribute(
                    "catalog-form-label",
                    "required"
                );
            } else {
    ...
    catalog.locationAllElements.forEach((element) => {
        element.addEventListener("click", function () {
            ...
            const countryInput = document.querySelectorAll(
                `${catalog.formName} .js-smarty-country`
            );
            if (userCountry !== "unknown") {
                countryInput.forEach((input) => {
                    const country = countryList.filter(
            ...
            if (catalog.locationValue !== "URL") {
                catalog.locationValue = element.getAttribute(
                    catalog.locationSelector
                );
            }
    ...
    // EVENT - Check for #hash or ?lightbox=value in URL
    if (
        pageURL.indexOf("#catalog") > 0 ||
        pageURL.indexOf("#Catalog") > 0 ||
        window.location.search.match(/lightbox\=catalog/i)
    ) {
    ...
    // EVENT - Handle submit of required form
    document
        .querySelector(".js-catalog-button-required")
        .addEventListener("click", function (e) {
            e.preventDefault();
            postFormCatalog();
        });
    ...
    document
        .querySelector(".js-catalog-button-optional")
        .addEventListener("click", function (e) {
            e.preventDefault();
            postFormOptional();
        });
    ...
    function pushToMarketoCatalogOptional(formName) {
        let checkboxesToCopyArray = document.querySelectorAll(
            `${catalog.formName} [mrkto-checkbox]`
        );
        checkboxesToCopyArray.forEach((checkbox) => {
            let mrktoCheckboxSelector = checkbox.getAttribute("mrkto-checkbox");
            if (checkbox.checked) {
                document.querySelector(mrktoCheckboxSelector).value =
                    checkbox.value;
            }
        });

        let travelFrequency = document.querySelectorAll(
            `${formName} [name='travel-frequency']`
        );
        travelFrequency.forEach((radio) => {
            if (radio.checked) {
                document.querySelector("#temp15CatTravelFrequency").value =
                    radio.value;
            }
        });
    }
    ...
    function postFormCatalog() {
        ...
        toggleButtonDisabled(
            document.querySelector(".js-catalog-button-required")
        );
    ...
    function postFormOptional() {
        ...
        toggleButtonDisabled(
            document.querySelector(".js-catalog-button-optional")
        );
...
async function modalEnews() {
    ...
    enews.phoneElementLabel = document.querySelector(
        '[for~="form-enews-phone"]'
    );
    enews.locationSelector = "js-enews-location";
    enews.locationFirstElement = document.querySelector(
        `[${enews.locationSelector}]`
    );
    enews.locationAllElements = document.querySelectorAll(
        `[${enews.locationSelector}]`
    );
    ...
    function smsCheckbox() {
        enews.smsElement.addEventListener("input", function () {
            if (this.checked) {
                ...
                enews.phoneElementLabel.setAttribute(
                    "enews-form-label",
                    "required"
                );
            } else {
    ...
    enews.locationAllElements.forEach((element) => {
        element.addEventListener("click", function (e) {
            ...
            if (enews.locationValue !== "URL") {
                enews.locationValue = element.getAttribute(
                    enews.locationSelector
                );
            }
    ...
    // EVENT - Check for #hash or ?lightbox=value in URL
    if (
        pageURL.indexOf("#enews") > 0 ||
        pageURL.indexOf("#eNews") > 0 ||
        window.location.search.match(/lightbox\=enews/i)
    ) {
    ...
    // EVENT - Handle submit of required form
    document
        .querySelector(".js-enews-button-required")
        .addEventListener("click", function (e) {
            e.preventDefault();
            postFormEnews();
        });
    ...
    function postFormEnews() {
        ...
        toggleButtonDisabled(
            document.querySelector(".js-enews-button-required")
        );
...
async function modalMessage() {
    ...
    message.smsElementParent = document.querySelector(
        ".js-sms-checkbox-message"
    );
    message.smsElement = document.querySelector("[js-message-sms]");
    message.phoneElementInput = document.querySelector(
        "[js-message-phone-toggle]"
    );
    message.phoneElementLabel = document.querySelector(
        '[for~="form-message-phone"]'
    );
    message.locationSelector = "js-message-location";
    message.locationFirstElement = document.querySelector(
        `[${message.locationSelector}]`
    );
    message.locationAllElements = document.querySelectorAll(
        `[${message.locationSelector}]`
    );
    ...
    function smsCheckbox() {
        message.smsElement.addEventListener("input", function () {
            if (this.checked) {
                ...
                message.phoneElementLabel.setAttribute(
                    "message-form-label",
                    "required"
                );
            } else {
    ...
    // EVENT - Check for #hash or ?lightbox=value in URL
    if (
        pageURL.indexOf("#message") > 0 ||
        window.location.search.match(/lightbox\=message/i)
    ) {
        message.locationFirstElement.click();
    }
    ...
    // EVENT - Handle submit of required form
    document
        .querySelector(".js-message-button-required")
        .addEventListener("click", function (e) {
            e.preventDefault();
            postFormmessage();
    ...
    function postFormmessage() {
        ...
        toggleButtonDisabled(
            document.querySelector(".js-message-button-required")
        );
...
async function modalRefer() {
    ...
    refer.phoneElementLabel = document.querySelector(
        '[for~="form-refer-phone"]'
    );
    refer.locationFirstElement = document.querySelector(
        `[${refer.locationSelector}]`
    );
    refer.locationAllElements = document.querySelectorAll(
        `[${refer.locationSelector}]`
    );
    ...
    refer.smsElement.addEventListener("input", function () {
        if (this.checked) {
            ...
            refer.phoneElementLabel.setAttribute(
                "refer-form-label",
                "required"
            );
        } else {
    ...
    refer.locationAllElements.forEach((element) => {
        element.addEventListener("click", function () {
            const countryInput = document.querySelectorAll(
                `${refer.formName} .js-smarty-country`
            );
            if (userCountry !== "unknown") {
                countryInput.forEach((input) => {
                    const country = countryList.filter(
    ...
    // EVENT - Check for #hash or ?lightbox=value in URL
    if (
        pageURL.indexOf("#refer") > 0 ||
        window.location.search.match(/lightbox\=refer/i)
    ) {
        refer.locationFirstElement.click();
    }
    ...
    // EVENT - Handle submit of required form
    document
        .querySelector(".js-refer-button-required")
        .addEventListener("click", function (e) {
            e.preventDefault();
            postFormRefer();
        });
    ...
    function postFormRefer() {
        ...
        toggleButtonDisabled(
            document.querySelector(".js-refer-button-required")
        );
...
async function modalDigital() {
    ...
    digital.smsElementParent = document.querySelector(
        ".js-sms-checkbox-digital"
    );
    digital.smsElement = document.querySelector("[js-digital-sms]");
    digital.phoneElementInput = document.querySelector(
        "[js-digital-phone-toggle]"
    );
    digital.phoneElementLabel = document.querySelector(
        '[for~="form-digital-phone"]'
    );
    digital.locationSelector = "js-digital-location";
    digital.locationFirstElement = document.querySelector(
        `[${digital.locationSelector}]`
    );
    digital.locationAllElements = document.querySelectorAll(
        `[${digital.locationSelector}]`
    );
    ...
    function smsCheckbox() {
        digital.smsElement.addEventListener("input", function () {
            if (this.checked) {
                ...
                digital.phoneElementLabel.setAttribute(
                    "digital-form-label",
                    "required"
                );
            } else {
    ...
    // EVENT - Check for #hash or ?lightbox=value in URL
    if (
        pageURL.indexOf("#digital") > 0 ||
        window.location.search.match(/lightbox\=digital/i)
    ) {
        digital.locationFirstElement.click();
    }
    ...
    // EVENT - Handle submit of required form
    document
        .querySelector(".js-digital-button-required")
        .addEventListener("click", function (e) {
            e.preventDefault();
            postFormdigital();
        });
    ...
    function postFormdigital() {
        ...
        toggleButtonDisabled(
            document.querySelector(".js-digital-button-required")
        );
        ...
        MktoForms2.whenReady(function () {
            ...
            let checkboxesToCopyArray = document.querySelectorAll(
                `${digital.formName} [mrkto-checkbox]`
            );
            checkboxesToCopyArray.forEach((checkbox) => {
                let mrktoCheckboxSelector =
                    checkbox.getAttribute("mrkto-checkbox");
                if (checkbox.checked) {
                    document.querySelector(mrktoCheckboxSelector).value =
                        checkbox.value;
                }
    ...
    function marketoSubmitFormDigital() {
        ...
        window.open(
            "https://issuu.com/natural-habitat-adventures/docs/catalog-2024-2025-natural-habitat-adventures?fr=xKAE9_zU1NQ",
            "_blank"
        );
    }
}
...
function modalYoutube() {
    ...
    document.addEventListener("click", function (event) {
        if (
            youtubeModal.contains(event.target) ||
            (event.target === youtubeModal && youtubeModal.open)
        ) {
            closeYoutubeModal();
        }
    });
...
function ebookCTAs() {
    ...
    async function eBookModal(id) {
        ...
        eBook.smsElementParent = document.querySelector(
            ".js-sms-checkbox-ebook"
        );
        eBook.smsElement = document.querySelector("[js-ebook-sms]");
        eBook.phoneElementInput = document.querySelector(
            "[js-ebook-phone-toggle]"
        );
        eBook.phoneElementLabel = document.querySelector(
            '[for~="form-ebook-phone"]'
        );
        eBook.phoneElementLabelRequired = document.querySelector(
            '[for~="form-ebook-phone"] span'
        );
        ...
        function smsCheckbox() {
            eBook.smsElement.addEventListener("input", function () {
                if (this.checked) {
                    ...
                    eBook.phoneElementLabel.setAttribute(
                        "ebook-form-label",
                        "required"
                    );
        ...
        // EVENT - Handle submit of required form
        document
            .querySelector(".js-ebook-submit-button")
            .addEventListener("click", function (e) {
                e.preventDefault();
                postFormEbook();
            });
        ...
        function copyEbookFieldsToMarketoForm() {
            let fieldsToCopyArray = document.querySelectorAll(
                `${eBook.formName} [mrkto-field]`
            );
            fieldsToCopyArray.forEach((field) => {
                let mrktoFieldSelector = field.getAttribute("mrkto-field");
                if (field.value) {
                    document.querySelector(mrktoFieldSelector).value =
                        field.value;
                }
            });
        }
        ...
        function postFormEbook() {
            ...
            toggleButtonDisabled(
                document.querySelector(".js-ebook-submit-button")
            );
            // Check if marketo form has been loaded, if not retry with limits
            if (checkMarketoForm() === false && retryAttempt < retryLimit) {
                initMarketoForm(function () {
            ...
            MktoForms2.whenReady(function () {
                ...
                document.querySelector("#referringWebsite").value =
                    utmObject.referringWebsite || "";
                document.querySelector("#recenteBookRequest").value =
                    marketoCampaignName;
                ...
                setTimeout(() => {
                    document
                        .querySelector(".js-ebook-goto-success button")
                        .click();
                }, 2100);
            });
        }
    ...
    function removeOmOverlay() {
        setTimeout(function () {
            try {
                document
                    .querySelector("html")
                    .classList.remove("om-position-popup");
                document
                    .querySelector("body")
                    .classList.remove("om-effect-overlay");
            } catch (error) {}
        }, 1500);
    }
    ...
    document.addEventListener("om.Campaign.load", (event) => {
        let omCampaign = document.querySelector(
            "#om-" + event.detail.Campaign.id
        );
        ...
        if (event.detail.Campaign.view === "optin") {
            if (window.location.pathname.includes("/blog/")) {
                ...
                let isModalOpenOrVideoPlaying =
                    callOpenModalAndVideoFunctions();
...
try {
    hideShowPricingAccordionsForAdmin();
} catch (error) {
    console.error(error);
}
...
try {
    document.addEventListener("DOMContentLoaded", (event) => {
        replaceMIPImagesInPricingAccordions();
    });
} catch (error) {
    console.error(error);
}
...
try {
    showBookOrCatalogOnMobileNav();
} catch (error) {
    console.error(error);
}

try {
    setSessionsForBackToTripBar();
} catch (error) {
    console.error(error);
}

try {
    clearOMSessionStorage();
} catch (error) {
    console.error(error);
}

try {
    if (window.location.pathname.includes("know-before-you-go")) {
        pageVisitCounter("itin-kbyg", "/know-before-you-go/");
    }
} catch (error) {
    console.error(error);
}
...
try {
    if (Cookies.get("itin") !== "hide") {
        pageVisitCounter("itin", "/");
    }
} catch (error) {
    console.error(error);
}
