// to do : rewrite component using json to make it update in real time
// to do : write test
function tripFinder() {
    let urlString;
    ...
    function datePlaceholderListener(placeholder, datepicker) {
        ...
        if (datepicker === ".js-datepicker-start") {
            datepickerDiv.addEventListener("change", function () {
                ...
                document
                    .querySelector(".js-datepicker-end")
                    .setAttribute("min", searchStart);
            });
        }
    ...
    function filterCountries(classArray) {
        ...
        resetValue.value = "";
        $("#countries-dropdown")
            .find("[trip-finder~='label']")
            .text("By Country");
        // Hide
        $(classArray).each(function () {
            let thisTarget =
                "#countries-dropdown [trip-finder='item']:not(." + this + ")";
    ...
    function filterWildlife(classArray) {
        ...
        $(classArray).each(function () {
            let thisTarget =
                "#wildlife-dropdown [trip-finder='item']:not(." + this + ")";
            $(thisTarget).css("color", "#ccc");
            $(thisTarget).css("pointer-events", "none");
        });
    }
    ...
    function filterInterests(classArray) {
        ...
        $(classArray).each(function () {
            let thisTarget =
                "#interest-dropdown [trip-finder='item']:not(." + this + ")";
            $(thisTarget).css("color", "#ccc");
            $(thisTarget).css("pointer-events", "none");
        });
    }
    ...
    $("[trip-finder='item']").on("click", function () {
        ...
        let thisValue = $(this).attr("trip-finder-link");
...
function hideDatePlaceholder(placeholderDiv) {
    placeholderDiv.classList.add("hide-date-placeholder");
}

//show date input field
function showDateInput(datepickerDiv) {
...
function homepageCarousel() {
    var currentSlide = 0,
        ...
        slider = new Object(),
        viewport = new Object(),
        sets = document.querySelectorAll("[slider='set']"),
        slides = document.querySelectorAll("[slider='slide']"),
        slideWindow,
        slideAmount;

    // FUNCTION - Set SlideWidth, Set Slide Amount
    function setSliderSettings() {
        slideWindow = $(".js-slide-amount");
        slideAmount = $(slideWindow).width();
        viewport.width = $(window).width();
        $("[slider='slide']").first().outerWidth();
    }

    // FUNCTION - Set Slider Start & Last, Number Each Slide, Set CurrentSlide
    function setSliderStart() {
        slider.first = 0;
        slider.last = slides.length - 1;
        slides.forEach((slide, index) => {
            slide.setAttribute("num", index);
        });
        sets.forEach((set, index) => {
            if (set.matches(".js-start")) {
                slider.start = slides.length / 3;
                currentSlide = slider.start;
            }
        });
    }

    // FUNCTION - Go to Slide (num)
    function gotoSlide(target, start) {
        if (target < slider.first || target > slider.last) {
            return;
        }

        let slidePX = slideAmount * -target + "px";
        // SCROLL PAGE TO SPECIFIC SPOT
        const mqSmall = window.matchMedia("(max-width : 767px)");
        const mqMedium = window.matchMedia(
            "(min-width : 768px) AND (max-width : 1023px)"
        );
        const mqLarge = window.matchMedia("(min-width : 1024px)");
        const selectorWrapper = document.querySelector(".js-slider-swipable");

        if (!start) {
            if (mqSmall.matches) {
                selectorWrapper.scrollIntoView({
                    behavior: "smooth",
                });
            } else if (mqMedium.matches || mqLarge.matches) {
                selectorWrapper.previousElementSibling.scrollIntoView({
                    behavior: "smooth",
                });
            }
        }
        ...
        if (activeSlide) {
            activeSlide.classList.remove("js-active-slide");
        }
        currentSlide = target;
        let sliderParent = document.querySelector(
            "[slider='slide'][num='" + target + "']"
        ).parentElement;

        while (
            sliderParent &&
            (!sliderParent.hasAttribute("slider") ||
                sliderParent.getAttribute("slider") != "set")
        ) {
            sliderParent = sliderParent.parentElement;
        }
        ...
        if (
            ...
            let getSet = Number(sliderParent.getAttribute("setnum"));
            if (getSet !== currentSet) {
                currentSet = getSet;
                setActiveForSelector(currentSet);
            }
        }

        document.querySelectorAll("[slider='set']").forEach(function (slider) {
            slider.style.transform = "translate3d(" + slidePX + ",0,0)";
        });

        document.querySelectorAll("[slider='slide']").forEach(function (slide) {
            let slideNumber = slide.getAttribute("num");
            slideNumber = parseInt(slideNumber);
            if (slideNumber === currentSlide) {
                slide.classList.add("js-active-slide");
            }
        });
    }

    // FUNCTION - Init Window Resizer Listener
    ...
    const enableSwipe = () => {
        ...
        // the js-active-slide class initializes the gesture zone
        // after the first swipe, we rely on the num attribute to move left & right through the lineup
        // which can be observed in the handleGesture function
        let gestureZone = [
            ...document.getElementsByClassName("js-active-slide"),
        ];

        // function that captures & updates the x/y coordinates where the user's fingers first touch the screen
        const touchStart = function (event) {
            touchstartX = event.changedTouches[0].screenX;
            touchstartY = event.changedTouches[0].screenY;
        };

        // function that captures & updates the x/y coordinates where the user's fingers leave the screen
        ...
        function handleGesture(touchstartX, touchstartY, touchendX, touchendY) {
            const xDirection = touchendX - touchstartX;
            const yDirection = touchendY - touchstartY;
            if (Math.abs(xDirection) > Math.abs(yDirection)) {
                // if xDirection is greater than 0, the user swiped right, meaning the next item to the left
                // should become visible (see currentSlide - 1)
                // on the flipside, if xDirection is less than 0, the user swiped left, meaning the next item
                // to the right should become visible (see currentSlide + 1)
                if (xDirection > 0) {
                    const target = Number(currentSlide) - 1;
                    gotoSlide(target);
                    gestureZone = [
                        ...document.querySelectorAll(`[num="${target}"]`),
                    ];
                } else {
                    const target = Number(currentSlide) + 1;
                    gotoSlide(target);
                ...
                if (gestureZone[0]) {
                    // remove event listener so that it doesn't double up
                    gestureZone[0].removeEventListener(
                        "touchstart",
                        touchStart,
                        false
                    );
                    ...
                    // add event listener
                    gestureZone[0].addEventListener(
                        "touchstart",
                        touchStart,
                        false
                    );
            ...
            // if the yDirection is greater than the xDirection, we know that the user's vector force was greater
            // in the y or up/down direction than in the x or left/right direction, indicating that they are
            // more than likely just scrolling the page, and we should not do anything to the carousel
            else if (Math.abs(xDirection) < Math.abs(yDirection)) {
                if (yDirection > 0) return "down";
                else return "up";
            }
            // similarly, if xDirection === yDirection, then the user tapped the screen and we will let the
            // click event on the image handle what the user is intending to do
            else return "tap";
        }

        // handles first initializion of event listeners on the first gesture zone,
        // one it's initalized, the functions that are invoked will be recursive and handle
        // assinging and calling themselves
        function intializeEventListener() {
            gestureZone[0].removeEventListener("touchstart", touchStart, false);
            gestureZone[0].removeEventListener("touchend", touchEnd, false);
            gestureZone[0].addEventListener("touchstart", touchStart, false);
            gestureZone[0].addEventListener("touchend", touchEnd, false);
        }
    ...
    // EVENT : Go to Previous Slide
    document
        .querySelector(".js-slider-prev")
        .addEventListener("click", function () {
            let target = Number(currentSlide) - 1;
            gotoSlide(target);
        });
    // EVENT : Go to Next Slide
    document
...
async function setUserCountry() {
    try {
        let response = await fetch(
            "https://ipinfo.io/json?token=5ddc6962f801d5"
        );
